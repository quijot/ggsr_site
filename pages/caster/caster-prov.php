...
<div class="well">
  ...
  if (isset($_POST['submit'])) {
    if (!verify_captcha())
      echo '<h2 class="text-danger">CAPTCHA no superado.</h2>';
    elseif (empty($id_ep))
    ...
    elseif (!file_exists($caster_dir . $id_ep)) {
  ...
  ?>
    ...
    <form class="form-horizontal" id="prov" method="post" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>">
      ...
      <input type="hidden" name="action" value="caster_prov">
    </form>
  <?php
  }
  ?>
...
<div class="well">
  ...
  <p>El caster se encuentra alojado en el laboratorio del
    <a href="http://www.fceia.unr.edu.ar/gps" title="Grupo de Geodesia Satelital de Rosario">Grupo de Geodesia Satelital
